/**
 * Created by WH1709040 on 2017/12/14
 */
...
@Component({
    ...
    templateUrl: './room-cabinet.component.html',
    styleUrls: ['./room-cabinet.component.scss'],
    providers: [MachineService],
    animations: [
        fadeLeftIn
    ]
})
export class RoomCabinetComponent implements OnInit, OnDestroy {
            ...
            if (data['data'].cabinetSet) {
                ...
                item.cabinetDatas.forEach(item => {
                    $this.$webSocket.stompClient
                        .subscribe('/itomTopic/alarmMaxLevel/' + item.cabinetId,
                            response => { // 订阅/topic/getResponse 目标发送的消息。这个是在控制器的@SendTo中定义的。
        ...
        this.http.get(`/itom/cabinet/queryServers/${cabinetId}`).subscribe(data => {
            ...
            if (data['data'].rackServers) {
                item.servicers.forEach(item => {
                    that.$webSocket.stompClient
                        .subscribe('/itomTopic/alarmMaxLevel/' + item.serverName + '(' + item.serverIp + ')', response => {
